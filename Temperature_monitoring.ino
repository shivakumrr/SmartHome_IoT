...
// DHT Sensor - GPIO 4 = D1 on ESP-12E NodeMCU board
const int DHTPin = 4;

// Lamp - LED - GPIO 5 = D2 on ESP-12E NodeMCU board
const int buzzer = 5;
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    */
    if (client.connect("espClient_room2")) {
      Serial.println("connected");  
      // Subscribe or resubscribe to a topic
      // You can subscribe to more topics (to control more LEDs in this example)
      client.subscribe("room2/buzzer");
   } else {
...
void loop() {
  ...
  if (now - lastMeasure > 1000) {
    ...
    client.publish("room2/temp", temperatureTemp);
